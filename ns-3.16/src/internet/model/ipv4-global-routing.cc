    ...
    case UDP_PROT_NUMBER:
      {
        ...
        tupleValue /= (m_ipv4->GetObject<Node>()->GetId() + 1);   //div can avoid classifing odd and even
        break;
      }
    case TCP_PROT_NUMBER:
      {
        ...
        NS_LOG_INFO ("Found TCP proto and header: " <<
                       tcpHeader.GetSourcePort () << ":" <<
                       tcpHeader.GetDestinationPort ());
        tupleValue += tcpHeader.GetSourcePort ();   //Removed Source Port dependency (Radhika)
        tupleValue += tcpHeader.GetDestinationPort (); //Removed Destination Port Dependency (Radhika)

        tupleValue /= (m_ipv4->GetObject<Node>()->GetId() + 1);   //div can avoid classifing odd and even
        //tupleValue += m_ipv4->GetObject<Node>()->GetId();              //Added router id nonce (Radhika)

        //Adding flow id dependency
        MyPriorityTag tag;
        bool peeked;
        peeked = ipPayload->PeekPacketTag(tag);
        if(peeked)
        {
  ...
  if (rtentry != 0)
    {
      ...
      NS_LOG_INFO(Simulator::Now().GetSeconds()<<" "<<"["<<m_ipv4->GetObject<Node>()->GetId()<<"]"<<" Routing packet to "<<rtentry->GetDestination()<<" on interface "<< m_ipv4->GetInterfaceForDevice (rtentry->GetOutputDevice()) );
